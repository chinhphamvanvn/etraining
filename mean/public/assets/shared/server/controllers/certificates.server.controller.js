...
exports.certificateByMember = function(req, res) {

    Certificate.findOne({member:req.member._id}).populate('user', 'displayName').populate('course').populate('authorizer').populate('edition').populate('member').exec(function (err, certificate) {
      if (err) {
        return next(err);
      } else if (!certificate) {
