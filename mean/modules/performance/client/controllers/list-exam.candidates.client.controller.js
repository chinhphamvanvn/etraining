(function () {
    ...
    function ExamCandidatesController($scope,$state, $filter, $compile, Authentication, AdminService, GroupsService, schedule, $timeout, $location, $window,DTOptionsBuilder, DTColumnDefBuilder, Notification, $q, ExamCandidatesService, $translate,treeUtils, _) {
      ...
      ExamCandidatesService.byExam({examId:vm.schedule.exam},function(data) {
     ...
     function selectStudents() {
         ...
         _.each(users,function(user) {
                 ...
                 if (!exist) {
                     ...
                     candidate.registered = new Date();
                     candidate.$save(function() {
                         Notification.success({ message: '<i class="uk-icon-check"></i> Candidate '+candidate.user.displayName + 'enroll successfully!'});
                         vm.students.push(candidate);
                     }, function(errorResponse) {
                         Notification.error({ message: errorResponse.data.message, title: '<i class="uk-icon-ban"></i> Candidate '+user.displayName + 'failed to enroll successfully!' });
                     });
                 }
         })
     }
     ...
     function suspend(candidate) {
         if (candidate.status=='withdrawn') {
             Notification.error({ message: '<i class="uk-icon-ban"></i> Cannot suspend withdraw candidate!' });
             return;
         }
         if (candidate.enrollmentStatus=='completed') {
             Notification.error({ message: '<i class="uk-icon-ban"></i> Cannot suspend candidate which completed the exam!' });
             return;
         }
         candidate.status = 'suspended';
         candidate.$update(
           function() {
             Notification.success({ message: '<i class="uk-icon-check"></i> Candidate suspend successfully!'})},
           function() {
             Notification.success({ message: '<i class="uk-icon-check"></i> Candidate suspend failed!' });
         })
     }
     
     function activate(candidate) {
         if (candidate.status=='withdrawn') {
             Notification.error({ message: '<i class="uk-icon-ban"></i> Cannot activate withdraw candidate!' });
             return;
         }
         if (candidate.enrollmentStatus=='completed') {
             Notification.error({ message: '<i class="uk-icon-ban"></i> Cannot activate candidate which completed the exam!' });
             return;
         }
         candidate.status = 'active';
         candidate.$update(
           function() {
             Notification.success({ message: '<i class="uk-icon-check"></i> Candidate activate successfully!'})},
           function() {
             Notification.success({ message: '<i class="uk-icon-check"></i> Candidate activate failed!' });
         })
     }     
  }
