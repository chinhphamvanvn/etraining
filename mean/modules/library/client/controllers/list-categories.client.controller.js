(function () {
    ...
    angular
      .module('library')
      .controller('LibraryCategoriesController', LibraryCategoriesController);

    LibraryCategoriesController.$inject = [ '$scope', '$state', '$stateParams', '$timeout', '$location', '$window', 'GroupsService', 'Notification', 'treeUtils', '_'];

    function LibraryCategoriesController( $scope, $state, $stateParams, $timeout, $location, $window, GroupsService, Notification, treeUtils, _) {
      var vm = this;
      vm.create = create;
      vm.remove = remove;
      vm.rename = rename;
      ...
      function create(node) {
       // if not is null, then try to get the active node
          if (!node) {
              var tree = $("#libTree").fancytree("getTree");
