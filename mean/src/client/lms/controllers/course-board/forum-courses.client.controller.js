(function(UIkit) {
  ...
  function CoursesForumController($scope, $state, $window, Authentication, $timeout, course, member, Notification, ForumTopicsService, ForumsService, ForumPostsService, $translate, _) {
    ...
    vm.forum = ForumsService.byCourse({
      courseId: vm.course._id
    }, function() {
      vm.topics = ForumTopicsService.byForum({
    ...
    function expand(topic) {
      topic.expand = true;
      topic.posts = ForumPostsService.byTopic({
        topicId: topic._id
      }, function() {
    ...
    function editTopic(topic) {
      UIkit.modal.prompt($translate.instant('MODEL.TOPIC.CONTENT') + ':', '', function(val) {
        topic.content = val;
        topic.$update(function() {}, function(errorResponse) {
          Notification.error({
            message: errorResponse.data.message,
            title: '<i class="uk-icon-ban"></i> Forum topic updated  error!'
          });
        });
      });
    }
    ...
    function removeTopic(topic) {
      UIkit.modal.confirm($translate.instant('COMMON.CONFIRM_PROMPT'), function() {
        topic.$remove(function() {
          ...
          Notification.error({
            message: errorResponse.data.message,
            title: '<i class="uk-icon-ban"></i> Forum topic removed  error!'
          });
        });
      });
    }
    ...
    function replyTopic(topic) {
      UIkit.modal.prompt($translate.instant('MODEL.POST.CONTENT') + ':', '', function(val) {
        ...
        post.$save(function() {
          ...
          Notification.error({
            message: errorResponse.data.message,
            title: '<i class="uk-icon-ban"></i> Forum podyrf created  error!'
          });
        });
      });
    }
    ...
    function editPost(post) {
      UIkit.modal.prompt($translate.instant('MODEL.POST.CONTENT') + ':', '', function(val) {
        post.content = val;
        post.$update(function() {}, function(errorResponse) {
          Notification.error({
            message: errorResponse.data.message,
            title: '<i class="uk-icon-ban"></i> Forum post updated  error!'
          });
        });
      });
    }
    ...
    function removePost(post) {
      UIkit.modal.confirm($translate.instant('COMMON.CONFIRM_PROMPT'), function() {
        post.$remove(function() {
          ...
          Notification.error({
            message: errorResponse.data.message,
            title: '<i class="uk-icon-ban"></i> Forum post removed  error!'
          });
        });
      });
    }
    ...
    function createForum() {
      ...
      forum.$save(function() {
        ...
        Notification.error({
          message: errorResponse.data.message,
          title: '<i class="uk-icon-ban"></i> Forum activation  error!'
        });
      });
    }
    ...
    function createTopic() {
      UIkit.modal.prompt($translate.instant('MODEL.TOPIC.CONTENT') + ':', '', function(val) {
        ...
        topic.$save(function() {
          ...
          Notification.error({
            message: errorResponse.data.message,
            title: '<i class="uk-icon-ban"></i> Forum topic created  error!'
          });
        });
      });
