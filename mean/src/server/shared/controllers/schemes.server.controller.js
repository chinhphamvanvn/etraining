...
exports.list = function(req, res) {
  GradeScheme.find().sort('-created').populate('user', 'displayName').populate('edition').exec(function(err, schemes) {
    if (err) {
      return res.status(400).send({
        message: errorHandler.getErrorMessage(err)
      });
...
exports.gradeSchemeByEditionID = function(req, res) {
  ...
  }).sort('-created').populate('user', 'displayName').populate('edition').exec(function(err, scheme) {
    if (err || !scheme) {
...
exports.schemeByID = function(req, res, next, id) {
  ...
  GradeScheme.findById(id).populate('user', 'displayName').populate('edition').exec(function(err, scheme) {
    if (err) {
      return next(err);
    } else if (!scheme) {
