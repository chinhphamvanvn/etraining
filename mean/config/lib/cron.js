...
function countRegisterAccount() {
    User.count({created:{$gt:new Date(Date.now() - 24*60*60 * 1000)}}).exec(function(err,count)
    {
...
function countLoginAccount() {
    UserLog.count({tag:'LOGIN',created:{$gt:new Date(Date.now() - 24*60 *60* 1000)}}).exec(function(err,count)
    {
...
function countCourseMember() {
    CourseMember.count({status:'active',enrollmentStatus:'registered',created:{$gt:new Date(Date.now() - 24*60 *60* 1000)}}).exec(function(err,count)
    {
        var stat = new Stat({created:new Date(),count:count,category:'MEMBER_REGISTER'});
        stat.save();
    });
    CourseMember.count({status:'active',enrollmentStatus:'instudy',created:{$gt:new Date(Date.now() - 24*60 *60 * 1000)}}).exec(function(err,count)
    {
        var stat = new Stat({created:new Date(),count:count,category:'MEMBER_INSTUDY'});
        stat.save();
    });
    CourseMember.count({status:'active',enrollmentStatus:'completed',created:{$gt:new Date(Date.now() - 24*60 *60* 1000)}}).exec(function(err,count)
    {
